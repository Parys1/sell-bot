...
## Тестовая версия распознавания

- Сейчас бот распознает **один предмет на одном скриншоте**.
- Предмет определяется по шаблонам из `app/item_templates/`.
- Количество читается из правого верхнего угла через Tesseract OCR.
- Для Railway используется `Dockerfile`, чтобы автоматически установить `tesseract-ocr`.
